...
**The research question we have landed on is the following:** Does UFO encounter length increase each decade following the release of 1977 film "Close Encounters of the Third Kind" when separated by country? Our hope is that this will help examine how time impacts UFO reports worldwide.
...
**Null hypothesis:** There IS NOT a difference in mean encounter length between UFO encounters before and after 1977.

**Alternative hypothesis:** There IS a difference in mean encounter length between UFO encounters before and after 1977.
...
filter_cylinder_US_CA <- ufo_sightings |>
  filter(ufo_shape == "cylinder" & country == "us" | country == "ca") |>
  mutate(countr = ifelse(country == "us", 1, 0))

filter_circle_US_CA <- ufo_sightings |>
  filter(ufo_shape == "circle" & country == "us" | country == "ca")

median_cylinder_US_CA <- filter_cylinder_US_CA |>
  drop_na(encounter_length) |>
  summarize(median_time = median(encounter_length))

median_circle_US <- filter_circle_US_CA |>
  drop_na(encounter_length) |>
  summarize(median_time = median(encounter_length))
```

```{r}
cyl_null_dist_US_CA <- filter_cylinder_US_CA |>
  specify(response = encounter_length , explanatory = countr) |>
...
cyl_null_dist_US_CA |>
  ggplot(
    aes(x = stat)) +
  geom_histogram() +
  labs(x = "Median Encouter Length (Seconds)", y = "Count", title = "Distribution of Encounter Length for Cylinders in the US and Canada")
